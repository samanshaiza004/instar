...
description: What is closed, what is in progress, what is still an open claim, and what Instar deliberately does not define.
sidebar:
  order: 1
---
...
- **Phase 1**: real guest in a real native window, bounded bridge, host-owned
  crash surface, and layered renderer boundary.
...
protocol revision 9 introduced the `TextView` node and the commit-local
