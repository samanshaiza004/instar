...
description: Clone the repository, build the host and a guest, and produce a distribution archive locally.
sidebar:
  order: 1
---
...
The site in `website/` is an Astro project; the guide you are reading is
Starlight.
...
`npm run build` writes the static site to `website/dist/`, which is what
Netlify publishes. Search is a production-only feature — Pagefind indexes the
built output, so the dev server shows a notice in place of results.
