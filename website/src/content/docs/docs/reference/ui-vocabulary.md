---
title: UI vocabulary
description: The semantic nodes, layout intent, presence rules, style fields, and identity model a guest can express.
sidebar:
  order: 2
---
...
"Hidden" and "display none" are different questions on purpose: one reserves
the space and one does not, and a guest that wants to keep a layout stable
while a control disappears needs to say which it means.
...
Font role is a role, not a family: the wire distinguishes system UI text from
monospace text and leaves the actual face to the host, which is the side that
...
For the byte-level encoding and the enforced limits, see the
[wire protocol reference](/docs/reference/protocol).
