...
/// `instar-text` is a sibling of `instar-ui`, not a layer beneath it.
///
/// The criterion that put it there is not "who owns the state" — the host owns
/// both — but whether the state's lifetime is subordinate to the semantic UI
/// tree. `FocusState` and `ScrollState` are: a `Scroll` *is* a tree node, focus
/// points at a `NodeKey`, and both die when the key stops being eligible. A
/// buffer is not: it must outlive any snapshot that happens to show it, and a
/// commit removing an editor node removes a presentation, not a document.
///
/// So the subsystem must not acquire an edge to anything that would make its
/// lifetime tree-shaped again.
#[test]
fn instar_text_depends_on_nothing_of_instars() {
    let tree = tree(
        &std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../instar-text"),
        &[],
    );
    let offenders: Vec<&String> = instar_crates(&tree)
        .iter()
        .filter(|crate_| crate_.as_str() != "instar-text")
        .cloned()
        .collect::<Vec<String>>()
        .leak()
        .iter()
    ...
    assert!(
        offenders.is_empty(),
        "instar-text links {offenders:?}.\n\
         It is a host resource subsystem composed by instar-host, not a layer \
         under the UI. An edge to instar-ui would make buffer lifetime \
         tree-shaped; an edge to instar-host or instar-window would make it \
         impossible to test the subsystem without a window.\n\n\
         Full tree:\n{tree}"
    );
}

/// And the edge back is absent rather than merely unused.
///
/// `instar-ui` measuring text through `instar-text` is a plausible-sounding
/// convenience that would invert the composition: the UI tree would then own
/// the thing it is only supposed to reference. If editor measurement makes
/// that genuinely awkward later, deleting this test is the deliberate act that
/// records the decision — which is the point of it existing before anyone is
/// tempted.
#[test]
fn instar_ui_does_not_depend_on_instar_text() {
    let tree = tree(
        &std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../instar-ui"),
        &[],
    );
    assert!(
        !instar_crates(&tree).iter().any(|c| c == "instar-text"),
        "instar-ui has taken a dependency on instar-text.\n\
         The composition is instar-host -> {{instar-ui, instar-text}}, and this \
         edge reverses it. If it is genuinely needed, that is a design change \
         to make on purpose rather than a dependency to add in passing.\n\n\
         Full tree:\n{tree}"
    );
}
