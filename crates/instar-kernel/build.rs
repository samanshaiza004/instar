...
fn main() {
    ...
    // The guest's bindings are generated from the kernel's own WIT world, so a
    // change to the world must rebuild the guest too.
    ...
    let target_dir = out_dir.join("kernel-spike-guest-target");
    ...
    let output = Command::new(cargo)
        ...
        .output()
        .expect("failed to spawn cargo for the kernel-spike guest fixture");
    ...
    if !output.status.success() {
        ...
        panic!(
            "building the kernel-spike guest fixture failed ({}).\n\
    ...
    let wasm = target_dir
        .join("wasm32-wasip2")
        .join("debug")
        .join("kernel_spike_guest.wasm");

    assert!(
        wasm.is_file(),
        "kernel-spike guest built successfully but no component at {}",
        wasm.display()
    );

    println!("cargo:rustc-env=KERNEL_SPIKE_GUEST_WASM={}", wasm.display());
}
